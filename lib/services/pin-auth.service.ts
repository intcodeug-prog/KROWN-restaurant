...
async function createSession(staff: any, deviceId: string | null) {
  const sql = getSql();
  const token = await createToken({ sub: staff.id, org: staff.organization_id, role: staff.role, branch: staff.assigned_branch_id || null, email: staff.email });
  await sql`
...
/** PIN-first authentication. Device proof is used when available, but is optional during the production access-recovery period so existing restaurants cannot be locked out. */
export async function authenticateByPinOnly(pin: string, deviceContext?: PinDeviceContext): Promise<AuthResult> {
  ...
  if (deviceContext) {
    const devices = await sql`
      ...
      FROM devices WHERE id=${deviceContext.deviceId} AND organization_id=${deviceContext.organizationId} AND branch_id=${deviceContext.branchId} LIMIT 1
    `;
    if (!devices.length || String((devices[0] as any).status) !== 'active') return { success: false, error: 'This device is not active. Activate it before signing in.' };
    const allowedRoles = Array.isArray((devices[0] as any).allowed_roles) ? (devices[0] as any).allowed_roles.map((r: unknown) => String(r).trim().toLowerCase()) : [];
    candidates = await sql`SELECT id,name,email,role,branch,assigned_branch_id,organization_id,pin_argon2,pin_code,status FROM staff WHERE organization_id=${deviceContext.organizationId} AND assigned_branch_id=${deviceContext.branchId} AND status='active' ORDER BY id`;
    candidates = candidates.filter((staff: any) => {
      const role = String(staff.role || '').trim().toLowerCase().replace(/\s+/g, '_');
      return !allowedRoles.length || allowedRoles.includes(role) || allowedRoles.includes(String(staff.role || '').trim().toLowerCase());
    });
  } else {
    // Emergency-safe recovery path: existing PIN users must be able to enter the POS
    // even if their browser has lost its device registration. A unique PIN is required.
    candidates = await sql`SELECT id,name,email,role,branch,assigned_branch_id,organization_id,pin_argon2,pin_code,status FROM staff WHERE status='active' ORDER BY id`;
  }
  ...
  if (matches.length > 1) return { success: false, error: 'This PIN is assigned to more than one account. Ask the KROWN team to resolve it.' };

  const staff = matches[0];
  return { success: true, ...(await createSession(staff, deviceContext?.deviceId || null)) };
}
