...
export interface StaffListOptions { limit?: number; offset?: number; search?: string; role?: string; status?: string; organizationId?: string; }
export interface StaffListResult { data: Staff[]; pagination: { limit:number; offset:number; hasMore:boolean; total:number; nextOffset:number|null; }; }

export async function listStaff(ctx:TenantContext, branchId?:string, options:StaffListOptions={}):Promise<Staff[]> {
  const result = await listStaffPaginated(ctx, branchId, options);
  return result.data;
}

export async function listStaffPaginated(ctx:TenantContext, branchId?:string, options:StaffListOptions={}):Promise<StaffListResult> {
  const sql=getSql();
  ...
  const requestedOrg=options.organizationId&&options.organizationId!=='all'?options.organizationId:null;
  ...
  const orgId=ctx.isSuperAdmin ? requestedOrg : ctx.organizationId;
  const rows=await sql`
    SELECT s.id,s.organization_id,s.name,s.email,s.phone,s.id_type,s.id_number,s.role,
      s.assigned_branch_id,s.status,s.avatar,s.created_at,s.updated_at,o.name as org_name,
      (SELECT ss.last_active_at FROM staff_sessions ss WHERE ss.staff_id=s.id ORDER BY ss.last_active_at DESC LIMIT 1) as last_login_at
    FROM staff s
    LEFT JOIN organizations o ON o.id=s.organization_id
    WHERE s.role!='super_admin'
      AND (${orgId}::text IS NULL OR s.organization_id=${orgId})
      AND (${effectiveBranch}::text IS NULL OR s.assigned_branch_id=${effectiveBranch})
      AND (${role}::text IS NULL OR s.role=${role})
      AND (${status}::text IS NULL OR s.status=${status})
      AND (${search}::text='' OR lower(s.name) LIKE '%' || ${search} || '%' OR lower(s.email) LIKE '%' || ${search} || '%')
    ORDER BY s.name ASC, s.id ASC
    LIMIT ${limit + 1} OFFSET ${offset}
  `;
  const hasMore=rows.length>limit;
  const data=(hasMore?rows.slice(0,limit):rows) as Staff[];
  return { data, pagination:{limit,offset,hasMore,total:offset+data.length+(hasMore?1:0),nextOffset:hasMore?offset+limit:null} };
}

export async function getStaff(ctx:TenantContext,staffId:string):Promise<Staff|null>{const sql=getSql(); if(!ctx.isSuperAdmin) await setTenantContext(sql,ctx.organizationId); const rows=ctx.isSuperAdmin ? await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE id=${staffId}` : await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE id=${staffId} AND organization_id=${ctx.organizationId}`; return rows.length?(rows[0] as Staff):null;}
...
export async function createStaff(ctx:TenantContext,input:{name:string;email:string;phone?:string;pin?:string;password?:string;idType?:string;idNumber?:string;role:string;branchId?:string;avatar?:string;}):Promise<Staff>{
  const sql=getSql(); await setTenantContext(sql,ctx.organizationId); const passwordHash=input.password?.trim()?await hashPassword(input.password.trim()):null; const hashedPin=input.pin?await hashPassword(input.pin):null; if(!passwordHash&&!hashedPin) throw new Error('A password or PIN is required'); const id=generateId(); const branchId=input.branchId&&input.branchId!=='all'?input.branchId:ctx.branchId||null;
  ...
  await logAudit(ctx.userId,'staff.create',{staffId:id,name:input.name,role:input.role},ctx.organizationId,ctx.branchId); const rows=await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE id=${id} AND organization_id=${ctx.organizationId}`; return rows[0] as Staff;
}

export async function updateStaff(ctx:TenantContext,staffId:string,updates:Partial<Pick<Staff,'name'|'email'|'phone'|'id_type'|'id_number'|'role'|'assigned_branch_id'|'avatar'>>):Promise<Staff>{const sql=getSql(); await setTenantContext(sql,ctx.organizationId); const existing=await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE id=${staffId} AND organization_id=${ctx.organizationId}`; if(!existing.length) throw new Error('Staff not found'); const fields:string[]=[]; const values:any[]=[]; for(const [key,value] of Object.entries(updates)) if(value!==undefined){fields.push(key);values.push(key==='assigned_branch_id'&&value==='all'?null:value);} if(!fields.length)return existing[0] as Staff; const setClauses=fields.map((f,i)=>`${f} = $${i+1}`).join(', '); values.push(staffId,ctx.organizationId); await sql(`UPDATE staff SET ${setClauses},updated_at=NOW() WHERE id=$${fields.length+1} AND organization_id=$${fields.length+2}`,values); await logAudit(ctx.userId,'staff.update',{staffId,fields},ctx.organizationId,ctx.branchId); const rows=await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE id=${staffId} AND organization_id=${ctx.organizationId}`; return rows[0] as Staff;}
