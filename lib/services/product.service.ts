...
export async function listProducts(ctx: TenantContext, branchId?: string): Promise<Product[]> {
...
export async function createProduct(ctx: TenantContext, input: { name:string; price:number; category:string; image?:string; available?:boolean; requiresKitchen?:boolean; description?:string; branchId?:string; linkedIngredientId?:string; deductFromInventory?:boolean; inventoryDeductAmount?:number; }): Promise<Product> {
  ...
  const countRows = targetBranchId
    ? await sql`SELECT COUNT(*)::int AS count FROM products WHERE organization_id = ${ctx.organizationId} AND branch_id = ${targetBranchId} AND available = true`
    : await sql`SELECT COUNT(*)::int AS count FROM products WHERE organization_id = ${ctx.organizationId} AND branch_id IS NULL AND available = true`;
...
export async function updateProduct(ctx: TenantContext, productId: string, updates: Partial<Pick<Product,'name'|'price'|'category'|'image'|'available'|'requires_kitchen'|'description'|'linked_ingredient_id'|'deduct_from_inventory'|'inventory_deduct_amount'>>): Promise<Product> {
  ...
  const product = existing[0] as Product;
  ...
  const rows=await sql`SELECT * FROM products WHERE id=${productId} AND organization_id=${ctx.organizationId}`; return rows[0] as Product;
}
export async function deleteProduct(ctx:TenantContext,productId:string):Promise<void>{
  const sql=getSql(); await setTenantContext(sql,ctx.organizationId);
  const existing=await sql`SELECT * FROM products WHERE id=${productId} AND organization_id=${ctx.organizationId}`;
  if(!existing.length) throw new Error('Product not found');
  const product=existing[0] as Product;
...
export async function toggleAvailability(ctx:TenantContext,productId:string):Promise<Product>{
  const sql=getSql(); await setTenantContext(sql,ctx.organizationId); const rows=await sql`SELECT * FROM products WHERE id=${productId} AND organization_id=${ctx.organizationId}`;
  if(!rows.length) throw new Error('Product not found'); const product=rows[0] as Product;
  if(ctx.branchId && product.branch_id && product.branch_id!==ctx.branchId) throw new Error('Forbidden: product belongs to another branch');
  const newAvailability=!product.available; await sql`UPDATE products SET available=${newAvailability},updated_at=NOW() WHERE id=${productId} AND organization_id=${ctx.organizationId}`;
  ...
  const updated=await sql`SELECT * FROM products WHERE id=${productId} AND organization_id=${ctx.organizationId}`; return updated[0] as Product;
}
export async function getRecipe(ctx:TenantContext,productId:string):Promise<ProductIngredient[]>{const sql=getSql();await setTenantContext(sql,ctx.organizationId);return await sql`SELECT * FROM product_ingredients WHERE product_id=${productId} AND organization_id=${ctx.organizationId}` as ProductIngredient[];}
export async function saveRecipe(ctx:TenantContext,productId:string,ingredients:{ingredientId:string;quantityPerUnit:number}[]):Promise<ProductIngredient[]>{
 const sql=getSql();await setTenantContext(sql,ctx.organizationId);const existing=await sql`SELECT * FROM products WHERE id=${productId} AND organization_id=${ctx.organizationId}`;if(!existing.length)throw new Error('Product not found');const product=existing[0] as Product;if(ctx.branchId&&product.branch_id&&product.branch_id!==ctx.branchId)throw new Error('Forbidden: product belongs to another branch');await sql`DELETE FROM product_ingredients WHERE product_id=${productId} AND organization_id=${ctx.organizationId}`;const saved:ProductIngredient[]=[];for(const ing of ingredients){const id=generateId();await sql`INSERT INTO product_ingredients (id,product_id,ingredient_id,quantity_per_unit,organization_id,branch_id,created_at) VALUES (${id},${productId},${ing.ingredientId},${ing.quantityPerUnit},${ctx.organizationId},${ctx.branchId},NOW())`;saved.push({id,product_id:productId,ingredient_id:ing.ingredientId,quantity_per_unit:ing.quantityPerUnit,organization_id:ctx.organizationId,branch_id:ctx.branchId||undefined,created_at:Date.now()});}await logAudit(ctx.userId,'product.save_recipe',{productId,ingredientCount:ingredients.length},ctx.organizationId,ctx.branchId);return saved;
}
