...
export async function POST(request: NextRequest) {
  try {
    ...
    // One login endpoint: first resolve platform admins, then restaurant staff.
    // Restaurant Admin is intentionally allowed to authenticate without a device.
    ...
    const directStaffResult = await authenticateStaff(email, password, undefined);
    ...
    if (!deviceId || !deviceProof?.challenge || !deviceProof?.signature) {
      return NextResponse.json({ data:null, error:'This computer is not activated for a restaurant or branch.' }, { status:403 });
    }
    ...
    if (!result.success || !result.token || !result.staff) return NextResponse.json({ data:null, error:result.error || 'Wrong email or password for this restaurant or branch' }, { status:401 });
    ...
    const deviceError = /device|challenge|signature|activated/i.test(message);
    return NextResponse.json({ data:null, error:deviceError ? 'This computer is not activated for this restaurant or branch.' : message }, { status:deviceError ? 403 : 500 });
  }
}
